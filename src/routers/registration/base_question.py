...
async def handle_payment_method_question(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if data["service"] == "1":
        ...
        await callback_query.message.answer(strings.PROBLEM_TYPE_QUESTION, reply_markup=kb.CLIENT_PROBLEM_INLINE_KEYBOARDS)
    else:
...
async def handle_message_end_registration(message: types.Message, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    await rq.set_registered_user(message.from_user.id, message.from_user.first_name, message.from_user.username, data)
    await state.clear()
