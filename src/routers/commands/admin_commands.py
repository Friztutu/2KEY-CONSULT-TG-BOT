...
async def handle_table_command(message: types.Message) -> None:
    types.ReplyKeyboardRemove()
    if message.from_user.id != ADMIN_USER_ID:
        await message.answer(strings.UNEXPECTED_COMMAND_STRING)
    else:
        await message.answer("Таблички!")


@router.message(Command("add_new_manager"))
async def handle_add_new_manager_command(message: types.Message) -> None:
    types.ReplyKeyboardRemove()
    if message.from_user.id != ADMIN_USER_ID:
        await message.answer(strings.UNEXPECTED_COMMAND_STRING)
    else:
...
async def handle_send_all_command(message: types.Message) -> None:
    types.ReplyKeyboardRemove()
    if message.from_user.id != ADMIN_USER_ID:
        await message.answer(strings.UNEXPECTED_COMMAND_STRING)
    else:
...
async def handle_send_to_user_command(message: types.Message) -> None:
    types.ReplyKeyboardRemove()
    if message.from_user.id != ADMIN_USER_ID:
        await message.answer(strings.UNEXPECTED_COMMAND_STRING)
    else:
